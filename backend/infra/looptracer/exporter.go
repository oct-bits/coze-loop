...
func uploadFile(ctx context.Context, fileName string, reader io.Reader, form map[string]string) error {
	...
	if err != nil {
		return errorx.Wrapf(err, "%s", fmt.Sprintf("create form file: %v", err))
	}

	if _, err = io.Copy(part, reader); err != nil {
		return errorx.Wrapf(err, "%s", fmt.Sprintf("copy file content: %v", err))
	}

	for key, value := range form {
		if err := writer.WriteField(key, value); err != nil {
			return errorx.Wrapf(err, "%s", fmt.Sprintf("write field %s: %v", key, err))
		}
	}

	if err := writer.Close(); err != nil {
		return errorx.Wrapf(err, "%s", fmt.Sprintf("close multipart writer: %v", err))
	}
	...
	if err != nil {
		...
		return errorx.Wrapf(err, "%s", fmt.Sprintf("http client UploadFile failed, err: %v", err))
	}
	if resp == nil || resp.GetCode() != 0 {
		logger.CtxErrorf(ctx, fmt.Sprintf("http client UploadFile failed, resp: %#v", resp))
		return errorx.Wrapf(err, "%s", fmt.Sprintf("http client UploadFile failed, resp: %#v", resp))
	}
